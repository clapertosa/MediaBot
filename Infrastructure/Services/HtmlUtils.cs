...
namespace Infrastructure.Services
{
    public class HtmlUtils : IHtmlUtils
    {
        ...
        public List<Media> GetMediaList(HtmlDocument htmlDoc)
        {
            ...
            /// </summary>
            var hasResults = htmlDoc.DocumentNode.SelectSingleNode(ImdbPaths.FindListTitlesHeader) != null;
        ...
        {
            var startIndex = url.Substring(1).IndexOf("/", StringComparison.Ordinal) + 2;
            var endIndex = url.LastIndexOf("/", StringComparison.Ordinal);
        ...
        {
            var pattern = @"(\d+)";
            int.TryParse(Regex.Match(title, pattern).Value, out var number);
            return number;
        }
        ...
        private string GetPosterOriginalSize(string imagePath)
        {
            ...
            var extensionStartIndex = imagePath.LastIndexOf(".", StringComparison.Ordinal) + 1;
            var sizesStartIndex = imagePath.IndexOf("@.", StringComparison.Ordinal);
            if (sizesStartIndex <= 0) return imagePath;
            sizesStartIndex += 2;
            var extension = imagePath[extensionStartIndex..imagePath.Length];
            return imagePath[..sizesStartIndex] + extension;
        }

        private List<Media> ParseTableResults(HtmlNode htmlNode)
        {
            var columns = htmlNode.SelectNodes(".//tr");
            var media = new List<Media>();

            foreach (var column in columns)
            {
                var id = GetId(column.SelectSingleNode(".//a").Attributes["href"].Value);
                var title = column.InnerText.Trim();
                var year = GetYearFromTitle(title);
                var posterPath = GetPosterOriginalSize(column.SelectSingleNode(".//img").Attributes["src"].Value);
                var m = new Media
                {
                    ImdbId = id,
                    Title = title,
        ...
        private Media ParseMediaPage(HtmlNode htmlNode, string url)
        {
            ...
            // Genres
            foreach (var genre in htmlNode.SelectNodes(ImdbPaths.Genres)) media.Genres.Add(genre.InnerText);

            // Actors
            var nodes = htmlNode.SelectNodes(ImdbPaths.ActorNamesAnchor);
            var actorsIndex = nodes.Count > 5 ? 5 : nodes.Count;
            for (var i = 0; i < actorsIndex; i++)
            {
                var actorFullName = nodes[i]?.InnerText;
                var actorUrl = $"{_imdbMediaUrl}{nodes[i].Attributes["href"].Value}";
